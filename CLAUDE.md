...
- **The bridge owns barge-in.** On providers with `capabilities.vadInterruptControl`, server-side auto-interrupt is disabled (`interrupt_response: false` injected unless the vad config sets it explicitly) and `CallSession` cancels/clears/truncates only when the `InterruptionController` allows — a blocked barge-in genuinely lets the agent keep talking, in-flight deltas of cancelled responses are dropped, and a guard-swallowed caller turn is answered after protected playback. Providers without the capability fall back to server auto-cancel (guard protects buffered audio only — asserted in `parity.test.ts`).
