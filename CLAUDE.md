...
- **Playback truth comes from marks.** A `tra:N` mark is interleaved after every audio delta; Twilio echoes marks at playout. `PlaybackTracker` classifies echoes (played vs flushed via clear-epochs). Never reintroduce wall-clock "the audio probably finished" timers — that bug class is why this design exists.
